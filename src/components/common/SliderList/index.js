            ...
            {listSlide.length > 0 && listSlide?.map((slide,idx) => 
                <SwiperSlide key={idx}>
                    <div className='slider-item'>
                        <img src={apiConfig.originalImgae(slide?.backdrop_path)} alt='img_poster' />
                        ...
                        <div className='info container'>
                            ...
                            <div className='rating d-flex align-items-center my-4'>
                                ...
                                <span>
                                    {slide.genres.length > 0 && (slide.genres).map((tag,idx) => <button type="button" className="btn btn-danger py-1 px-2 mx-2" key={idx}>{tag.name}</button>)}
                                </span>
                            </div>
